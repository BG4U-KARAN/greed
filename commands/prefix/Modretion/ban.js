const {
  ...
  module.exports = {
    ...
    description: "ban member from the server", // command description
    aliases: ["k"],
    permissions: {
    ...
    run: async (client, message, args) => {
      ...
      if (!page) {
        return message.reply({
          embeds: [
            embed.setDescription(
              `**<:cross_mark:1133343048827420772> I couldn't send you a Direct Message to confirm the ban. Please make sure your DMs are open.**`
            ),
          ],
          components: [],
        });
        ...
        switch (i.customId) {
          case "ban-yes":
            ...
            member
              .send({
                embeds: [
                  embed.setDescription(`You have been banned from ${guild.name}`),
                ],
              })
              .catch((er) => {
                ...
                return message.reply({
                  embeds: [
                    embed.setDescription(
                      `**<:cross_mark:1133343048827420772> I couldn't send member a Direct Message to confirm the ban. Please make sure your DMs are open.**`
                    ),
                  ],
                  components: [],
                });
